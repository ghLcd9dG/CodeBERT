...
def convert_examples_to_features(item):
    ...
    code_tokens, dfg = extract_dataflow(
        js['original_string'], parser, args.lang)
    ...
    def __init__(self, tokenizer, args, file_path=None, pool=None):
        ...
        if 'train' in file_path:
            for idx, example in enumerate(self.examples[:3]):
                ...
                logger.info("dfg_to_code: {}".format(
                    ' '.join(map(str, example.dfg_to_code))))
                logger.info("dfg_to_dfg: {}".format(
...
def train(args, model, tokenizer, pool):
    ...
    scheduler = get_linear_schedule_with_warmup(
        optimizer, num_warmup_steps=0, num_training_steps=len(train_dataloader)*args.num_train_epochs)
    ...
    for idx in range(args.num_train_epochs):
        ...
        results = evaluate(args, model, tokenizer,
                           args.eval_data_file, pool, eval_when_training=True)
...
def evaluate(args, model, tokenizer, file_name, pool, eval_when_training=False):
    ...
    query_dataloader = DataLoader(
        query_dataset, sampler=query_sampler, batch_size=args.eval_batch_size, num_workers=4)

    code_dataset = TextDataset(tokenizer, args, args.codebase_file, pool)
    code_sampler = SequentialSampler(code_dataset)
    code_dataloader = DataLoader(
        code_dataset, sampler=code_sampler, batch_size=args.eval_batch_size, num_workers=4)
